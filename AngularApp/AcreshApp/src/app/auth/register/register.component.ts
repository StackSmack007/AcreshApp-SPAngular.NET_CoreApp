...
function getNextItem<T>(arr: T[], currentItem: T): T {
  let index: number = arr.indexOf(currentItem);
  index = (index + 1 === arr.length) ? 0 : index + 1
  return arr[index]
}

function getPreviousItem<T>(arr: T[], currentItem: T): T {
  let index: number = arr.indexOf(currentItem);
...
export class RegisterComponent {
  ...
  nextPicture() {
    this.rf.controls.avatarPicture.setValue(getNextItem(avImages[this.gender], this.avatarImg));
  }

  prevPicture() {
    this.rf.controls.avatarPicture.setValue(getPreviousItem(avImages[this.gender], this.avatarImg));
  }
